/****************
   (Feb07) Add the 4th function: Feeding (combine "_feeding")
...
const float turbidWaterChanging = 3; //(?1.5) Check later! Ideally(scientifically) set "< 4.1-0.3=3.8 V".
...
//"Feeding"
Servo myservo;
const float turbidFeeding = 1.0; //(?) not too turbid to feed
const int feedHr1 = 11; //the pre-set feeding hour (in 24)
...
void loop() {
  ...
  if (turbidityVol > turbidFeeding) { //NOT very turbid (with large turbidity voltage)-> judge turbidity first
    if (now.Minute() == 0 && now.Second() < 30 && (now.Hour() == feedHr1 || now.Hour() == feedHr2)) //judge time then
